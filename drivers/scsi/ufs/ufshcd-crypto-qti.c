				       ...
				       struct ufshcd_lrb *lrbp)
{
	...
	if (req->bio->android_kabi_reserved1 != 0 && cnt >= 0 && unlikely(req->bio->android_kabi_reserved1 != bc->bc_dun[0])) {
		pr_err("[FBE]: dun_backup=%llx, bc_dun[0]=%llx\n", req->bio->android_kabi_reserved1, bc->bc_dun[0]);
		if (ufs_dclient && !dsm_client_ocuppy(ufs_dclient)) {
			dsm_client_record(ufs_dclient, "%llx, %llx\n", req->bio->android_kabi_reserved1, bc->bc_dun[0]);
