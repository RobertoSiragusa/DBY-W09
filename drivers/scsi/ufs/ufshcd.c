...
extern void record_db_data(struct bio *bio, int stage, u64 data);
					...
					bool in_irq)
{
	...
	if (lrbp && lrbp->cmd && lrbp->cmd->request && lrbp->cmd->request->bio && lrbp->cmd->request->bio->android_kabi_reserved2 && lrbp->ucd_prdt_ptr) {
		base_addr = (u64)le32_to_cpu(lrbp->ucd_prdt_ptr->base_addr);
		upper_addr = (u64)le32_to_cpu(lrbp->ucd_prdt_ptr->upper_addr);
		whole_addr = base_addr | (upper_addr << 32);
		src = (u64 *)phys_to_virt(whole_addr);
		record_db_data(lrbp->cmd->request->bio, 0, *src);
	}
