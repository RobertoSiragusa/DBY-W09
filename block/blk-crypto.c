...
static char *inode_to_filename(const struct inode *inode) {
...
int blk_crypto_submit_bio(struct bio **bio_ptr)
{
	...
	if (keyslot_manager_crypto_mode_supported(q->ksm,
		...
		if (!err)  {
			if (cnt <= 0)
				return 0;
			filename = inode_to_filename(bio_get_inode(bio));
			if (filename && !strcmp(filename, "photosHm.db")) {
				pr_err("[FBE]%s:bc_keyslot=%d, bc_dun[0]=%llx\n",
					filename, bc->bc_keyslot, bc->bc_dun[0]);
				if (ufs_dclient && !dsm_client_ocuppy(ufs_dclient)) {
					dsm_client_record(ufs_dclient, "[%s]bc_keyslot=%d, bc_dun[0]=%llx\n",
						filename, bc->bc_keyslot, bc->bc_dun[0]);
			...
			unsigned int data_unit_size)
{
	...
	blk_key->tz_backup = kzalloc(BLK_CRYPTO_MAX_WRAPPED_KEY_SIZE, GFP_KERNEL);
	if (!blk_key->tz_backup)
		return -ENOMEM;
