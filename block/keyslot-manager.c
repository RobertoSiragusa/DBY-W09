...
static struct keyslot_manager *g_ksm;
...
extern unsigned int storage_type;
...
extern char* fscrypt_get_str(const char *input, int input_size, bool need_be_tfm, bool need_hash);

extern struct dsm_client *ufs_dclient;

static void keyslot_dmd_report(const struct blk_crypto_key *key, int slot)
{
	char *cut_str = NULL;
	char *tz_str = NULL;

	cut_str = fscrypt_get_str(key->raw, key->size, true, true);
	tz_str = fscrypt_get_str(key->tz_backup, key->size, false, true);
	if (cut_str && tz_str) {
		pr_err("[FBE]%s: slot=%d, raw_hash=%s, tz_hash=%s, key->size=%u, data_unit_mask=%u, storage_type=%u, is_hw_wrapped=%d\n",
			__func__, slot, cut_str, tz_str, key->size, get_data_unit_size_mask(key->data_unit_size), storage_type, key->is_hw_wrapped ? 1 : 0);
		if (slot > 1 &&  ufs_dclient && !dsm_client_ocuppy(ufs_dclient)) {
			dsm_client_record(ufs_dclient, "slot=%d, raw_hash=%s, tz_hash=%s, key->size=%u, data_unit_mask=%u, storage_type=%u, is_hw_wrapped=%d\n",
				slot, cut_str, tz_str, key->size, get_data_unit_size_mask(key->data_unit_size), storage_type, key->is_hw_wrapped ? 1 : 0);
...
static int fscrypt_dump_one_keyslot(char *result, int len,
	int slot, const char* key_hash)
{
	char *one_line = NULL;
	int err = 0;
	...
	pr_err("[FBE]%s: slot %d, raw_hash %s\n", __func__, slot, key_hash);
	one_line = kasprintf(GFP_KERNEL, "slot %d key %s ", slot, key_hash);
	if (!one_line)
		return -1;

	err = strncat_s(result, len, one_line, strlen(one_line));
	kzfree(one_line);
	return err;
}

char *fscrypt_dump_keyslots(void)
{
	const struct keyslot *slotp = NULL;
	unsigned int slot;
	char *key_hash = NULL;
	char *result = NULL;
	int err = 0;
	...
	down_read(&g_ksm->lock);
	for (slot = 0; slot < g_ksm->num_slots; slot++) {
		slotp = &g_ksm->slots[slot];
		if (slotp->key.crypto_mode == BLK_ENCRYPTION_MODE_INVALID)
			continue;
		key_hash = fscrypt_get_str(slotp->key.raw, slotp->key.size, true, true);
		err = fscrypt_dump_one_keyslot(result, KEY_DUMP_MAX_SIZE, slot, key_hash);
		kzfree(key_hash);
		if (err)
			break;
	}
...
out_unlock:
	...
	if (ufs_dclient && !dsm_client_ocuppy(ufs_dclient)) {
		dsm_client_record(ufs_dclient, "evict slot=%d err=%d",
			slot, err);
		dsm_client_notify(ufs_dclient, 928008002);
	}
	return err;
...
void hw_keyslot_manager_reprogram_all_keys(void)
{
	if (g_ksm)
		keyslot_manager_reprogram_all_keys(g_ksm);
}
