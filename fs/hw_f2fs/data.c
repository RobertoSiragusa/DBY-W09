...
#define DB_HEAD_DATA 0x66206574694c5153

static void db_data_dmd_report(struct bio *bio)
{
	u64 *db_data= (u64 *)bio->android_kabi_reserved2;

	if (db_data[2] != DB_HEAD_DATA) {
		pr_err("[FBE] corrupt data %llx %llx %llx\n", db_data[0], db_data[1], db_data[2]);
		if (f2fs_dclient && !dsm_client_ocuppy(f2fs_dclient)) {
			dsm_client_record(f2fs_dclient, "corrupt data %llx %llx %llx",
				db_data[0], db_data[1], db_data[2]);
			dsm_client_notify(f2fs_dclient, 928005001);
		}
	}

	kfree(db_data);
	bio->android_kabi_reserved2 = 0;
}

void record_db_data(struct bio *bio, int stage, u64 data)
{
	u64 *reserve_data = NULL;
	if (!bio || stage > 2 || stage < 0)
		return;

	reserve_data = (u64 *)bio->android_kabi_reserved2;
	reserve_data[stage] = data;
}
...
static void f2fs_read_end_io(struct bio *bio)
{
	...
	if (first_page && bio && bio->android_kabi_reserved2) {
		src = (u64 *)page_to_virt(first_page);
		record_db_data(bio, 2, *src);
		db_data_dmd_report(bio);
	}
...
extern bool is_target_file(const char *name);
				      ...
				      pgoff_t first_idx, bool for_write)
{
	...
	if (dentry && dentry->d_name.name && is_target_file(dentry->d_name.name) && first_idx == 0) {
		pr_err("[FBE]%s: set db flag.\n", dentry->d_name.name);
		bio->android_kabi_reserved2 = (u64)kzalloc(24, GFP_KERNEL);
	}
	if (dentry)
		dput(dentry);
