...
extern char *fscrypt_dump_inode(struct inode *inode);
extern char *fscrypt_dump_keyslots(void);
extern void fscrypt_dump_direct_keys(struct super_block *sb);
...
bool is_target_file(const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(target_filename); i++) {
		if (strcmp(name, target_filename[i]) == 0)
			return true;
	}
	return false;
}

static void db_rename_dmd_report(const struct dentry *old_dentry,
	const struct dentry *new_dentry)
{
	char *inode_dump = NULL;
	char *slot_dump = NULL;

	if (!is_target_file(old_dentry->d_name.name))
		return;

	pr_err("[FBE] rename to %s\n", new_dentry->d_name.name);

	inode_dump = fscrypt_dump_inode(d_inode(old_dentry));
	slot_dump = fscrypt_dump_keyslots();

	if (!(f2fs_dclient && !dsm_client_ocuppy(f2fs_dclient)))
		return;

	dsm_client_record(f2fs_dclient, "%s ", new_dentry->d_name.name);
	if (inode_dump)
		dsm_client_record(f2fs_dclient, "%s ", inode_dump);
	if (slot_dump)
		dsm_client_record(f2fs_dclient, "%s\n", slot_dump);
	dsm_client_notify(f2fs_dclient, 928005004);

	kzfree(inode_dump);
	kzfree(slot_dump);

	fscrypt_dump_direct_keys(d_inode(old_dentry)->i_sb);
}
			...
			unsigned int flags)
{
	...
	db_rename_dmd_report(old_dentry, new_dentry);
